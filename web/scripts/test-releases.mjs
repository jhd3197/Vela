...
function verifyCleanupTarget() {
  if (
    path.dirname(temp) !== path.resolve(os.tmpdir()) ||
    !path.basename(temp).startsWith('vela-release-ui-')
  ) {
...
execFileSync(
  ...
  [
    '-c',
    "from scripts.fixture_apps import APPS; import shutil,sys; shutil.copytree(APPS / 'health', sys.argv[1])",
    folder,
  ],
  { cwd: root, windowsHide: true },
);
const manifest = JSON.parse(await fs.readFile(path.join(folder, 'app.json'), 'utf8'));
...
execFileSync(
  ...
  [
    '-c',
    "import pathlib,sys,zipfile; root=pathlib.Path(sys.argv[1]); z=zipfile.ZipFile(sys.argv[2],'w'); [z.write(p,p.relative_to(root)) for p in root.rglob('*') if p.is_file()]; z.close()",
    folder,
    archive,
  ],
  { windowsHide: true },
...
try {
  ...
  await dialog.getByRole('button', { name: 'Install release' }).click();
  await dialog.waitFor({ state: 'hidden' });
  await page.goto(base + '/app/health');
  const frame = page.frameLocator('iframe');
  await frame.getByRole('tab', { name: 'Habits' }).click();
  await frame.locator('#habitName').fill('Keep through release');
  ...
  await rollback.getByRole('button', { name: 'Restore release' }).click();
  await rollback.waitFor({ state: 'hidden' });
  await page.goto(base + '/app/health');
  await frame.getByRole('tab', { name: 'Habits' }).click();
  await frame.getByText('Keep through release', { exact: true }).waitFor();
  assert.deepEqual(errors, []);
  console.log(
    'PASS: pinned Health install, permission review, ZIP update without hub rebuild, retained data, matching rollback, responsive review',
  );
} finally {
  await browser?.close();
  server.kill();
