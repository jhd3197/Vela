...
execFileSync('openssl', ['req', '-x509', '-newkey', 'rsa:2048', '-nodes', '-keyout', key, '-out', cert, '-days', '1', '-subj', '/CN=127.0.0.1', '-addext', 'subjectAltName=IP:127.0.0.1'], { windowsHide: true, stdio: 'ignore' });
const pem = await fs.readFile(cert);
const pin = createHash('sha256').update(createPublicKey(pem).export({type:'spki',format:'der'})).digest('base64');
const python = process.env.VELA_TEST_PYTHON || path.join(root, process.platform === 'win32' ? '.venv/Scripts/python.exe' : '.venv/bin/python');
const server = spawn(python, ['scripts/serve-connection-fixtures.py', '--cert', cert, '--key', key], {cwd:root, windowsHide:true, stdio:['ignore','pipe','pipe']});
let output = '', browser;
...
try {
  for (let i=0; i<100; i++) {
    ...
    try { await new Promise((resolve,reject) => https.get(base+'/api/health', {ca:pem}, r => { r.resume(); r.on('end', resolve); }).on('error',reject)); break; }
    catch { if(i===99) throw Error(output); await new Promise(r=>setTimeout(r,100)); }
  }
  browser = await chromium.launch({headless:true, channel:process.env.VELA_BROWSER_CHANNEL || 'chrome', args:[`--ignore-certificate-errors-spki-list=${pin}`]});
  const pages = [];
  ...
  const [a,b] = pages, fa=a.frameLocator('iframe'), fb=b.frameLocator('iframe');
  const legacy = [{id:'legacy-one',name:'Imported habit',target:3,color:'#aabbcc',days:{}}];
  await a.evaluate(value=>localStorage.setItem('vela.health.habits.v1',JSON.stringify(value)),legacy);
  ...
  assert.deepEqual(JSON.parse(await a.evaluate(()=>localStorage.getItem('vela.health.habits.v1'))),legacy);
  await a.getByText('Import earlier Health data',{exact:true}).click();
  await fa.getByText('Imported habit',{exact:true}).waitFor();
  await fb.getByText('Imported habit',{exact:true}).waitFor();
  const add = async (frame,name) => { await frame.locator('#habitName').fill(name); await frame.locator('#addHabit').click(); await frame.locator('#syncStatus').filter({hasText:'Saved to your engine'}).waitFor(); };
  ...
  const shots=path.join(root,'docs/screenshots/connections'); await fs.mkdir(shots,{recursive:true});
  for (const page of pages) await page.screenshot({path:path.join(shots,`health-${page.viewportSize().width}.png`)});
  ...
  await a.frameLocator('iframe').getByRole('button',{name:'View details for fixture:latest'}).click();
  ...
  console.log('PASS: authenticated TLS, independent desktop/mobile clients, retained migration, cross-client updates, conflict recovery, backup/restore, cookie resume, Ollama read bridge');
} finally {
