...
export default function ReleaseImport() {
  ...
  return (
    <details className="release-import">
      <summary>Import apps &amp; refresh catalog</summary>
      <p>
        Choose a release ZIP from this device, or enter an app folder on the engine. You’ll review
        its permissions and data changes before installation.
      </p>
      <label>
        Release archive
        <input
      ...
      <p>
        {catalog?.cached ? 'Using cached catalog.' : 'Catalog ready.'}{' '}
        {catalog?.releases?.length ?? 0} pinned releases.
      </p>
      {catalog?.error && (
        <p role="status">
          Refresh unavailable: {catalog.error} Cached entries and installed apps remain available.
        </p>
      )}
