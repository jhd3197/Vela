      ...
      >
        ...
        <div className="drawer-body">
          <div className="drawer-row">
            ...
            {live.running && <span className="drawer-running-note">Running locally inside the hub</span>}
          </div>
          ...
          <section className="drawer-section">
            ...
            <dl className="drawer-facts">
              ...
              <dt>Access</dt>
              <dd>{isProcessApp(app) ? 'Trusted native code · runs as your user' : app.schemaVersion === 2 ? (app.capabilities?.includes('storage') ? 'Own persistent app data' : 'No engine data access') : 'Trusted legacy app'}</dd>
              {webApp ? (
                <>
                  <dt>Type</dt>
                  <dd>{app.schemaVersion === 2 ? 'Static app — isolated view' : 'Legacy web app (PWA)'}</dd>
                  <dt>Installed</dt>
          ...
          {isProcessApp(app) && (
            <section className="drawer-section">
              ...
              {statusError ? (
                ...
                <pre ref={logRef} className="drawer-log mono">{status.logs}</pre>
              ) : (
                <p className="drawer-log-empty">
                  {status?.running ? 'No log output yet.' : 'Logs appear here while the app is running.'}
                </p>
              )}
            </section>
          )}
        ...
        <div className="drawer-footer">
          {app.upgradeAvailable && <p>A new SDK version is available. Your earlier browser records and installed package are retained.</p>}
          {upgradeError && <p role="alert">{upgradeError}</p>}
          <div className="actions">
            {app.upgradeAvailable && <button className="btn btn-primary" disabled={upgrading} onClick={async () => { setUpgrading(true); try { await api.upgrade(app.id); await refreshApps(); } catch (failure) { setUpgradeError(failure.message); } finally { setUpgrading(false); } }}>Upgrade to SDK version</button>}
            {app.supported && live.installed && (
              <button className="btn btn-primary" onClick={openEmbedded}>
                Open
              </button>
            )}
            {app.supported && !live.installed && (
              <button className="btn btn-primary" disabled={busy} onClick={() => onAction(app.id, 'install')}>
                Install
              </button>
            )}
            {app.supported && live.installed && !live.running && (
              <button className="btn" disabled={busy} onClick={() => onAction(app.id, 'launch')}>
                Launch
              </button>
            )}
            {app.supported && live.running && isProcessApp(app) && (
              <button className="btn" disabled={busy} onClick={() => onAction(app.id, 'stop')}>
                Stop
              </button>
            )}
            {app.supported && live.installed && (
              <button className="btn btn-danger" disabled={busy} onClick={() => onAction(app.id, 'uninstall')}>
                Uninstall
              </button>
            )}
          </div>
