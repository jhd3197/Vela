...
import { artworkKey, monogram, ID_MARKS, CATEGORY_MARKS } from '../web/src/appArtworkKey.js';
import { parseColor, toOklch, tileTones, tileColor } from '../web/src/appTint.js';
...
test('an app with its own artwork keeps it, whatever its category says', () => {
  ...
  assert.deepEqual(artworkKey({ id: 'health', category: 'connected', name: 'Health' }), {
    kind: 'id',
    key: 'health',
  });
});
...
test('categories resolve however the manifest capitalises them', () => {
  for (const category of CATEGORY_MARKS) {
    ...
    assert.deepEqual(artworkKey({ id: 'x', category: category.toUpperCase() }), {
      kind: 'category',
      key: category,
    });
  }
...
test('a connected service is identified by its initial, by prefix or category', () => {
  ...
  assert.deepEqual(artworkKey({ id: 'anything', category: 'connected', name: 'jellyfin' }), {
    kind: 'monogram',
    key: 'J',
  });
...
test('anything unrecognised gets the generic mark', () => {
  for (const app of [null, undefined, {}, { id: 'nope', category: 'mystery' }]) {
...
test('colours are read from hex and rgb, and refused otherwise', () => {
  ...
  for (const bad of ['', null, 'rebeccapurple', '#12345', 'rgb(1,2)', 'nonsense']) {
...
test('tile tones normalise lightness so every app carries the same weight', () => {
  ...
  const declared = ['#7c4dee', '#9184d9', '#2bb6d8', '#21a377', '#75798c', '#8b5cf6', '#a59c8b'];
  const marks = declared.map((color) => toOklch(parseColor(tileTones(color).mark)).L);
  const washes = declared.map((color) => toOklch(parseColor(tileTones(color).wash)).L);
  ...
  assert.ok(spread(marks) < 0.03, `marks should land together, spread was ${spread(marks)}`);
  assert.ok(spread(washes) < 0.03, `washes should land together, spread was ${spread(washes)}`);
  ...
  for (const color of declared) {
    ...
    assert.ok(toOklch(parseColor(tones.mark)).L < toOklch(parseColor(tones.wash)).L);
    assert.ok(toOklch(parseColor(tones.markDark)).L > toOklch(parseColor(tones.washDark)).L);
  }
});
...
test('tones stay inside sRGB instead of clipping to neon', () => {
  ...
  assert.ok(toOklch(parseColor(grey.mark)).C < 0.04, 'a grey app must not become colourful');
});
